...
void prepareCylinder(float altura, float radius, int lados) {
	...
	//fill arrays
	glBufferData(GL_ARRAY_BUFFER, 36, vertexB, GL_STATIC_DRAW);
	
	for (size_t i = 0; i < 36; i+=3){
		z = cos(alfa); x = sin(alfa); 
		vertexB[i] = z; 	vertexB[i + 1] = y; vertexB[i + 2] = x;
		alfa += M_PI / 9;
...
void renderScene(void) {
	...
	glClearColor(0.391f, 0.0f, 0.0f, 0.0f);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	glLoadIdentity();
	glLightfv(GL_LIGHT0, GL_POSITION, pos);
	gluLookAt(camX, camY, camZ,	0.0, 0.0, 0.0,	0.0f, 1.0f, 0.0f);
	drawCylinder();		// End of frame
	glutSwapBuffers();
}
